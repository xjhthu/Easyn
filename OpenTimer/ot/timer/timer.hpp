...
class Timer {
  ...
  public:
    ...
    Timer& insert_frontier(Pin& pin);
    Timer& repower_gate(std::string, std::string);
    Timer& repower_gate_with_map(std::string, std::string,HashTable<std::string>& pin_name_map);
    ...
    float report_delay_between(const std::string& name1, const std::string& name2, Split m, Tran t);
    std::optional<float> direct_report_at(const std::string&, Split, Tran);
    void debug(const std::string&, Split, Tran);
    void update_at_pin(const std::string&, Split, Tran);
    void update_all();
    void run_all_tasks();
    std::optional<float> my_report_slack(const std::string&, Split, Tran);
  ...
  private:
    ...
    void _repower_gate_with_map(const std::string&, const std::string&,const HashTable<std::string>& pin_name_map);
