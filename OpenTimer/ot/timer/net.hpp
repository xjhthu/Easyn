...
class Rct {
  ...
  public:
    ...
    void print_ncap(const RctNode& node) {
        std::cout << "ncap for node: " << node.name() << "\n";
        for (int el = 0; el < MAX_TRAN; ++el) {
          for (int rf = 0; rf < MAX_SPLIT; ++rf) {
            std::cout << "el=" << el << " rf=" << rf << " ncap=" << node._ncap[el][rf] << "\n";
          }
          std::cout<<std::endl;
        }
    }
    void print_node_names() {
      std::cout << "RCTree Node names:" << std::endl;
...
class Net {
  ...
  public:
    ...
    bool is_net_root(const Pin* pin) const{
      return pin == _root;
    }
    ...
    void reset_spef_net() { _reset_spef_net(); }
    void update_rc_timing() {
        _update_rc_timing();
    }
