...
Timer& Timer::repower_gate_with_map(std::string gate, std::string cell,HashTable<std::string>& pin_name_map) {

  std::scoped_lock lock(_mutex);

  auto task = _taskflow.emplace([this, gate=std::move(gate), cell=std::move(cell), pin_name_map=pin_name_map] () {
    _repower_gate_with_map(gate, cell,pin_name_map);
  });
...
// Procedure: _repower_gate
void Timer::_repower_gate_with_map(const std::string& gname, const std::string& cname, const HashTable<std::string>& pin_name_map) {
  ...
  if(auto gitr = _gates.find(gname); gitr == _gates.end()) {
    ...
    for(auto pin : gate._pins) {
      FOR_EACH_EL(el) {
        assert(pin->cellpin(el));
        if(const auto cpin = cell[el]->cellpin( pin_name_map.at(pin->cellpin(el)->name.c_str()))) {
          std::cout<<pin_name_map.at(pin->cellpin(el)->name.c_str())<<"\n";
          pin->_remap_cellpin(el, *cpin);
        }
        else {
...
Net& Timer::_insert_net(const std::string& name) {
  auto result = _nets.try_emplace(name, name);
  Net& net = result.first->second;

  if (net.root() == nullptr){
    if (!net.pins().empty()){
      Pin* lastPin = net.pins().back();
      net.root(*lastPin);
    }
  }
...
std::optional<float> Timer::direct_report_at(const std::string& name, Split m, Tran t) {
...
void Timer::update_at_pin(const std::string& name, Split m, Tran t){
  if(auto itr = _pins.find(name); itr != _pins.end()) {
    for(auto arc : itr->second._fanin){
      _fprop_rc_timing(arc->_from);
      _fprop_slew(arc->_from);
      _fprop_delay(arc->_from);
      _fprop_at(arc->_from);
      _fprop_test(arc->_from);
    }
    _fprop_rc_timing(itr->second);
    _fprop_slew(itr->second);
    _fprop_delay(itr->second);
    _fprop_at(itr->second);
    _fprop_test(itr->second);
  }
}
...
float Timer::report_delay_between(const std::string& name1, const std::string& name2, Split m, Tran t){
  if(auto itr = _pins.find(name1); itr != _pins.end() ) {
    float totaldelay=-MAXFLOAT;
    for(auto arc : itr->second._fanin){
      for(auto arc1 : arc->_from._fanin){
        // std::cout<<name1<<' '<<arc1->name()<<' '<<name2<<std::endl;
        if((arc1->name()).find(name2)!=std::string::npos){
            totaldelay=mymax(totaldelay,*(arc1->_delay[ot::MAX][ot::FALL][ot::FALL])+*(arc->_delay[ot::MAX][ot::FALL][ot::FALL]));
            totaldelay=mymax(totaldelay,*(arc1->_delay[ot::MAX][ot::FALL][ot::RISE])+*(arc->_delay[ot::MAX][ot::RISE][ot::FALL]));
        }
      }
...
void Timer::debug(const std::string& name, Split m, Tran t)
{
  if(auto itr = _pins.find(name); itr != _pins.end() && itr->second._at[m][t]) {
    for(auto arc : itr->second._fanin){
      std::cout<<arc->name()<<' '<<*(arc->_delay[ot::MAX][ot::FALL][ot::FALL])<<' '<<*(arc->_from._at[ot::MAX][ot::FALL])<<' '<<*(arc->_to._at[ot::MAX][ot::FALL])<<'\n';
      std::cout<<arc->name()<<' '<<*(arc->_delay[ot::MAX][ot::RISE][ot::FALL])<<' '<<*(arc->_from._at[ot::MAX][ot::RISE])<<' '<<*(arc->_to._at[ot::MAX][ot::FALL])<<'\n';
      for(auto arc1 : arc->_from._fanin){
        std::cout<<arc1->name()<<' '<<*(arc1->_delay[ot::MAX][ot::FALL][ot::FALL])<<' '<<*(arc1->_from._at[ot::MAX][ot::FALL])<<'\n';
      }
      
    }
    if(auto pi = itr->second.primary_input(); pi){
      std::cout<<"pin "<<*(pi->_at[ot::MAX][ot::FALL])<<"\n";
    }
  }
}
...
void Timer::update_all(){
  _update_timing();
}

std::optional<float> Timer::my_report_slack(const std::string& pin, Split m, Tran t){
